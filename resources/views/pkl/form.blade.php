...
@section('content')
    <h6 class="text-uppercase">
        Pendaftaran Praktik Kerja Lapangan (PKL)</h6>
    <hr>
    @if (!$jadwal_pkl)
        <div class="alert alert-warning border-0 bg-warning alert-dismissible fade show py-2">
            <div class="d-flex align-items-center">
                ...
                <div class="ms-3">
                    ...
                    <div class="text-dark">
                        Jadwal kontrak pendaftaran PKL belum dibuka/sudah berakhir.
                    </div>
                </div>
            </div>
        </div>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            document.querySelectorAll(".btn-daftar").forEach(function(button) {
                button.addEventListener("click", function() {
                    ...
                    fetch("{{ route('pendaftaran-pkl.store') }}", {
                            method: "POST",
                            headers: {
                        ...
                        .then(data => {
                            if (data.success) {
                                alert(data.message);
                                window.location.href = "{{ route('pendaftaran-pkl.index') }}";
                            } else {
                                alert(data.message);
                            }
                        })
