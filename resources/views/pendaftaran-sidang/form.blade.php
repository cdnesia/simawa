@extends('layouts.app')
@section('content')
    <h6 class="text-uppercase">Sidang Akhir Skripsi</h6>
    <hr>
    @if (!$jadwal_kkn)
        <div class="alert alert-warning border-0 bg-warning alert-dismissible fade show py-2">
            <div class="d-flex align-items-center">
                <div class="font-35 text-dark"><i class='bx bx-info-circle'></i>
                </div>
                <div class="ms-3">
                    <h6 class="mb-0 text-dark">Mohon Maaf !!!</h6>
                    <div class="text-dark">
                        Jadwal pendaftaran Sidang Akhir belum dibuka/sudah berakhir.
                    </div>
                </div>
            </div>
        </div>
    @else
        <div class="row row-cols-1 row-cols-md-1 row-cols-lg-3 row-cols-xl-3">
            @forelse ($persyaratan as $item)
                <div class="col">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">{{ $item['nama_kegiatan'] }}</h5>
                        </div>
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item"><strong>Persyaratan</strong></li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">Minimal SKS
                                {{ $item['minimal_sks'] }}
                                @if ($jumlah_sks >= $item['minimal_sks'])
                                    <span class="badge bg-success rounded-pill">
                                        Memenuhi
                                    </span>
                                @else
                                    <span class="badge bg-danger rounded-pill">
                                        Tidak Memenuhi
                                    </span>
                                @endif
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">Minimal Semester
                                {{ $item['minimal_semester'] }}
                                <span class="badge bg-success rounded-pill">
                                    Memenuhi
                                </span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">Maksimal Nilai D
                                {{ $item['maksimal_nilai_d'] }}
                                @if ($jumlah_d <= $item['maksimal_nilai_d'])
                                    <span class="badge bg-success rounded-pill">
                                        Memenuhi
                                    </span>
                                @else
                                    <span class="badge bg-danger rounded-pill">
                                        Tidak Memenuhi
                                    </span>
                                @endif
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">Sudah Kontrak MK
                                Skripsi
                                @if ($sudah_kontrak_skripsi)
                                    <span class="badge bg-success rounded-pill">
                                        Memenuhi
                                    </span>
                                @else
                                    <span class="badge bg-danger rounded-pill">
                                        Tidak Memenuhi
                                    </span>
                                @endif
                            </li>
                        </ul>
                        <div class="card-footer">
                            @if ($jumlah_sks >= $item['minimal_sks'] && $jumlah_d <= $item['maksimal_nilai_d'] && $sudah_kontrak_skripsi)
                                <button type="submit" class="btn btn-primary btn-primary btn-sm btn-daftar"
                                    data-id="{{ $item['encrypted_id'] }}">
                                    Daftar
                                </button>
                            @else
                                <button type="button" class="btn btn-secondary btn-sm" disabled
                                    title="Belum memenuhi seluruh persyaratan pendaftaran">
                                    Daftar
                                </button>
                            @endif
                        </div>
                    </div>
                </div>
            @empty
                <div class="col">
                    <div class="alert alert-info">Belum ada persyaratan Sidang Akhir yang dikonfigurasi untuk program studi Anda.</div>
                </div>
            @endforelse
        </div>
    @endif
@endsection
@push('js')
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            document.querySelectorAll(".btn-daftar").forEach(function(button) {
                button.addEventListener("click", function() {
                    let encryptedId = this.getAttribute("data-id");
                    fetch("{{ route('pendaftaran-sidang-tugas-akhir.store') }}", {
                            method: "POST",
                            headers: {
                                "Content-Type": "application/json",
                                "X-CSRF-TOKEN": "{{ csrf_token() }}"
                            },
                            body: JSON.stringify({
                                id: encryptedId
                            })
                        })
                        .then(response => response.json())
                        .then(data => {
                            if (data.success) {
                                alert(data.message);
                                window.location.href = "{{ route('pendaftaran-sidang-tugas-akhir.index') }}";
                            } else {
                                alert(data.message);
                            }
                        })
                        .catch(error => {
                            alert("Terjadi kesalahan");
                        });

                });

            });

        });
    </script>
@endpush
